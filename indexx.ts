...
class Edificio {
  ...
  addDepartamentoToPiso(nombreDePiso: string, departamento: Departamento) {
    let pisoEncontrado = this.pisos.find((p) => p.nombre == nombreDePiso);
    return pisoEncontrado.pushDepartamento(departamento);
  }
  getDepartamentosByPiso(nombreDelPiso: string): Departamento[] {
    let pisoEncontrado = this.pisos.find((p) => p.nombre == nombreDelPiso);
...
console.log("un coñoemadre nada ","naah mentira");
